...
# --- Create a custom LWP::UserAgent for testing ---
{
	...
	sub get {
		...
		# version of what the Census API might return.
		my $content = '{"result": {"addressMatches": [{"dummy": "match"}]}}';
...
ok(exists $res1->{result}, "Result contains key 'result'");
ok(ref($res1->{result}{addressMatches}) eq 'ARRAY',
	"Result contains 'addressMatches' array");
...
is_deeply($res1, $res2, 'Second call returns cached result (same as first)');
